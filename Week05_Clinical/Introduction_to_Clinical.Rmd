...
```{r}
write.csv(clinic, "/home1/selinali/490_cluster/analysis_data/brca_clinical_data.csv", row.names = FALSE)
...
```{r}
clinic <- read.csv("/home1/selinali/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
plot(clinic$age_at_diagnosis, clinic$last_contact_days_to,
     main = "Age at diagnosis vs Last contact",
     xlab = "Age at diagnosis (days)",
     ylab = "Last contact (days)")
```
...
boxplot(clinic$age_at_diagnosis ~ clinic$gender,
        main = "Age by Gender",
        xlab = "Gender", ylab = "Age at diagnosis (days)")
```
...
```{r}
under35_over60_mask <- clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60
```
...
# The y axis represent the survival probability of the patients

# 2. What does the x axis represent?
#The x axis represents time since diagnosis in days.
...
#The old age group appears to have the worst within 3000 days, but young group appears to have the worst rate after 3000~4000 days.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
#15 years are about 5475 days. The green line is around 30% survival probability at that time.
...
#The p-value is 0.2 > 0.1 and 0.05, it is not statistically significant. This means there is no strong evidence that survival differs by age group in this dataset.
