...
keep_gene <- rowSums(count_mat >= 10) >= 20
count_mat <- count_mat[keep_gene, , drop = FALSE]

set.seed(1)
table(rna_clinical$definition_bin)
sel <- unlist(tapply(seq_len(nrow(rna_clinical)),
                     rna_clinical$definition_bin,
                     function(x) sample(x, min(150, length(x)))))
rna_clinical <- rna_clinical[sel, , drop = FALSE]
count_mat    <- count_mat[, rownames(rna_clinical), drop = FALSE]
